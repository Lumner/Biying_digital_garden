...
> 版本：2026-05-13  
> 来源：根据 `FDS/` 目录下当前 11 份 PPT/PDF 课件整理。  
> 定位：这份讲义不是逐页转写课件，而是把定义、抽象数据类型、算法思想、复杂度分析、实现模板和典型算例整理成可复习、可发布、可继续增补的 Markdown 文档。  
> 维护方式：后续新增课件时，优先补充到对应章节；若出现新主题，则在文末“增量补充区”登记来源并扩展为正式章节。

## 使用说明

这门课的主线是：用合适的数据组织方式，让算法能在可接受的时间和空间内完成任务。学习时不要只背接口名称，而要追问三件事：
...
- [资料来源索引](#资料来源索引)
- [全课知识结构](#全课知识结构)
- [0. 课程视角：为什么需要数据结构](#0-课程视角为什么需要数据结构)
- [1. 算法分析](#1-算法分析)
- [2. 抽象数据类型与线性表](#2-抽象数据类型与线性表)
- [3. 栈与队列](#3-栈与队列)
- [4. 树与二叉树](#4-树与二叉树)
- [5. 二叉搜索树](#5-二叉搜索树)
- [6. 优先队列与二叉堆](#6-优先队列与二叉堆)
- [7. 并查集](#7-并查集)
- [8. 线段树](#8-线段树)
- [9. 图与拓扑排序](#9-图与拓扑排序)
- [10. 全课复杂度速查](#10-全课复杂度速查)
- [11. 选结构的思考模板](#11-选结构的思考模板)
- [12. 增量补充区](#12-增量补充区)
